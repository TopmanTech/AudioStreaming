...
class MediaSyncServiceTest extends TestCase
{
    ...
    public function testSync(): void
    {
        ...
        self::assertDatabaseHas(Artist::class, ['name' => 'AudioStreaming']);

        // Albums should be created
        self::assertDatabaseHas(Album::class, ['name' => 'AudioStreaming Testing Vol. 1']);

        // Albums and artists should be correctly linked
        /** @var Album $album */
        $album = Album::query()->where('name', 'AudioStreaming Testing Vol. 1')->first();
        self::assertSame('AudioStreaming', $album->artist->name);
        ...
        self::assertSame('AudioStreaming', $song->album_artist->name);
    ...
    {
        config(['AudioStreaming.ignore_dot_files' => false]);
        $this->mediaService->sync();
        self::assertDatabaseHas(Album::class, ['name' => 'Hidden Album']);

        config(['AudioStreaming.ignore_dot_files' => true]);
        $this->mediaService->sync();
