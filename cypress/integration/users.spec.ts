context('User Management', () => {
  ...
  it('adds a user', () => {
    ...
    cy.findByTestId('add-user-form').within(() => {
      cy.get('[name=name]').should('be.focused')
        .type('Charles')
      cy.get('[name=email]').type('charles@AudioStreaming.test')
    ...
    cy.get('#usersWrapper [data-testid=user-card]:first-child').within(() => {
      ...
      cy.findByText('charles@AudioStreaming.test').should('be.visible')
      cy.findByTitle('User has admin privileges').should('be.visible')
    })
  })
  ...
  it('edits a user', () => {
    ...
    cy.findByTestId('edit-user-form').within(() => {
      ...
      cy.get('[name=email]').should('have.value', 'alice@AudioStreaming.test')
        .clear().type('adriana@AudioStreaming.test')
    ...
    cy.get('#usersWrapper [data-testid=user-card]:nth-child(2)').within(() => {
      ...
      cy.findByText('adriana@AudioStreaming.test').should('be.visible')
    })
  })
