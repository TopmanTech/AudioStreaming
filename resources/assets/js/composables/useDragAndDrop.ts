...
const getDragType = (event: DragEvent) => {
  return draggableTypes.find(type => event.dataTransfer?.types.includes(`application/x-AudioStreaming.${type}`))
}
...
export const useDraggable = (type: DraggableType) => {
  const startDragging = (event: DragEvent, dragged: Draggable) => {
    ...
    event.dataTransfer.setData(`application/x-AudioStreaming.${type}`, JSON.stringify(data))
...
export const useDroppable = (acceptedTypes: DraggableType[]) => {
  ...
  const getDroppedData = (event: DragEvent) => {
    ...
    try {
      return JSON.parse(event.dataTransfer?.getData(`application/x-AudioStreaming.${type}`)!)
    } catch (e) {
      ...
      switch (getDragType(event)) {
        case 'playlist':
          return playlistStore
            .byId(parseInt(event.dataTransfer!.getData('application/x-AudioStreaming.playlist'))) as T | undefined
        default:
          return
      }
    } catch (error) {
