<template>
  <aside :class="{ 'showing-pane': activeTab }">
    <div class="controls">
      ...
      <div class="bottom">
        <button
          v-AudioStreaming-tooltip.left
          :title="shouldNotifyNewVersion ? 'New version available!' : 'About AudioStreaming'"
          type="button"
          @click.prevent="openAboutAudioStreamingModal"
        >
          <Icon :icon="faInfoCircle" />
          <span v-if="shouldNotifyNewVersion" class="new-version-notification" />
        </button>

        <button v-AudioStreaming-tooltip.left title="Log out" type="button" @click.prevent="logout">
...
const openAboutAudioStreamingModal = () => eventBus.emit('MODAL_SHOW_ABOUT_AudioStreaming')
