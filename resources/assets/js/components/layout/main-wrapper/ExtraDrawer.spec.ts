...
new class extends UnitTestCase {
  ...
  protected test () {
    ...
    it('shows About AudioStreaming model', async () => {
      const emitMock = this.mock(eventBus, 'emit')
      this.renderComponent()

      await this.user.click(screen.getByRole('button', { name: 'About AudioStreaming' }))

      expect(emitMock).toHaveBeenCalledWith('MODAL_SHOW_ABOUT_AudioStreaming')
    })
