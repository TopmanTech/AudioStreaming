...
class MediaSyncService
{
    ...
    private function gatherFiles(string $path): array
    {
        return iterator_to_array(
            $this->finder->create()
                ...
                ->ignoreDotFiles((bool) config('AudioStreaming.ignore_dot_files')) // https://github.com/AudioStreaming/AudioStreaming/issues/450
                ->files()
                ->followLinks()
    ...
    private function setSystemRequirements(): void
    {
        if (!app()->runningInConsole()) {
            set_time_limit(config('AudioStreaming.sync.timeout'));
        }

        if (config('AudioStreaming.memory_limit')) {
            ini_set('memory_limit', config('AudioStreaming.memory_limit') . 'M');
        }
    }
