 ...
 * If this installation of AudioStreaming has a CDN_URL configured, use it as the base.
...
{
    $cdnUrl = trim(config('AudioStreaming.cdn.url'), '/ ');
...
{
    return $fileName ? public_path(config('AudioStreaming.album_cover_dir') . $fileName) : null;
}

function album_cover_url(?string $fileName): ?string
{
    return $fileName ? static_url(config('AudioStreaming.album_cover_dir') . $fileName) : null;
}

function artist_image_path(?string $fileName): ?string
{
    return $fileName ? public_path(config('AudioStreaming.artist_image_dir') . $fileName) : null;
}

function artist_image_url(?string $fileName): ?string
{
    return $fileName ? static_url(config('AudioStreaming.artist_image_dir') . $fileName) : null;
}

function AudioStreaming_version(): string
{
