...
class InitCommand extends Command
{
    ...
    private const DEFAULT_ADMIN_NAME = 'AudioStreaming';
    private const DEFAULT_ADMIN_EMAIL = 'admin@AudioStreaming.dev';
    private const DEFAULT_ADMIN_PASSWORD = 'AudioStreamingIsCool';
    private const NON_INTERACTION_MAX_DATABASE_ATTEMPT_COUNT = 10;

    protected $signature = 'AudioStreaming:init {--no-assets}';
    protected $description = 'Install or upgrade AudioStreaming';
    ...
    {
        $this->alert('AudioStreaming INSTALLATION WIZARD');
        $this->info(
            'As a reminder, you can always install/upgrade manually following the guide at '
            . config('AudioStreaming.misc.docs_url')
            . PHP_EOL
        );
        ...
        try {
            ...
            $this->components->error("Oops! AudioStreaming installation or upgrade didn't finish successfully.");
            $this->components->error('Please check the error log at storage/logs/laravel.log and try again.');
            $this->components->error('You can also visit ' . config('AudioStreaming.misc.docs_url') . ' for other options.');
        ...
        $this->info('AudioStreaming can now be run from localhost with `php artisan serve`.');
        ...
        if (Setting::get('media_path')) {
            $this->info('You can also scan for media now with `php artisan AudioStreaming:sync`.');
        }

        $this->info('Again, visit 📙 ' . config('AudioStreaming.misc.docs_url') . ' for more tips and tweaks.');

        $this->info(
            "Feeling generous and want to support AudioStreaming's development? Check out "
            . config('AudioStreaming.misc.sponsor_github_url')
            . ' 🤗'
        );

        $this->info('Thanks for using AudioStreaming. You rock! 🤘');
    ...
    private function setUpDatabase(): void
    {
        ...
        if ($config['DB_CONNECTION'] === 'sqlite-e2e') {
            ...
            $config['DB_DATABASE'] = $this->anticipate('DB name', ['AudioStreaming']);
            $config['DB_USERNAME'] = $this->anticipate('DB user', ['AudioStreaming']);
    ...
    private function maybeSetMediaPath(): void
    {
        ...
        while (true) {
            $path = $this->ask('Media path', config('AudioStreaming.media_path'));
    ...
    {
        $path = config('AudioStreaming.media_path');
